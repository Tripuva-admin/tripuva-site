          ...
          <div className="bg-white rounded-xl w-full max-w-5xl flex flex-col md:flex-row relative shadow-xl overflow-hidden max-h-[calc(100vh-32px)]">
            ...
            <div className="w-full md:w-1/2 flex flex-col max-h-[calc(100vh-250px-32px)] md:max-h-[calc(100vh-32px)]">
              <div className="flex-1 overflow-y-auto px-4 py-4 md:p-8">
                <div className="font-instrumentsans flex flex-col mb-3 md:mb-4">
                  ...
                  {pkg.agency && (
                    <div className="mt-2 flex items-start space-x-2">
                      ...
                      <div>
                        <p className="font-instrumentsans text-gray-700 font-medium">{pkg.agency.name}</p>
                ...
                <div className="space-y-3 mb-4 md:mb-6">
                  <div className="flex items-center text-gray-700">
                    <div className="flex items-center">
                      ...
                      <span className="font-instrumentsans font-medium text-sm md:text-base">Available Dates:</span>
                    </div>
                    {pkg.start_date_2 && Object.keys(pkg.start_date_2).some(date => new Date(date) >= new Date(new Date().setHours(0, 0, 0, 0))) && (
                      <span className="font-instrumentsans ml-3 text-sm text-[#1c5d5e] italic">Choose your departure</span>
                    )}
                  </div>

                  <div className="font-instrumentsans flex flex-wrap gap-2">
                    {pkg.start_date_2 && Object.keys(pkg.start_date_2).length > 0 ? (
                              ...
                              return (
                                <div key={date} className="flex flex-col items-center">
                                  <button
                                    ...
                                    className={`text-sm py-1 px-2.5 rounded-full transition-colors whitespace-nowrap ${isPast || isSoldOut
                                  ...
                                  {!isPast && (
                                    <span className={`text-xs mt-1 ${isSoldOut ? 'text-red-500' : 'text-gray-700'}`}>
                      ...
                      <span className="font-instrumentsans text-[#1c5d5e] text-sm">
                ...
                <div className="flex flex-wrap items-center gap-4 md:gap-6 mb-4 md:mb-6">
                  <div className="flex items-center text-gray-700">
                    ...
                    <span className="font-instrumentsans font-medium text-sm md:text-base">Duration:</span>
                    <span className="font-instrumentsans ml-1 text-gray-600 text-sm md:text-base">{pkg.duration} days</span>
                  </div>

                  <div className="flex items-center text-gray-700">
                    <Users className="h-4 w-4 md:h-5 md:w-5 mr-2 text-[#1c5d5e]" />
                    <span className="font-instrumentsans font-medium text-sm md:text-base">Group Size:</span>
                    <span className="font-instrumentsans ml-1 text-gray-600 text-sm md:text-base">{pkg.group_size} people</span>
                  </div>
                </div>
                ...
                <div className="bg-[#1c5d5e]/5 rounded-lg p-3 md:p-4 mb-4 md:mb-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 mb-2">
                    <div className="text-gray-700 text-sm md:text-base">
                      <span className="font-instrumentsans font-medium">Booking Advance: </span>
                      <span className="font-instrumentsans text-[#1c5d5e] font-semibold">₹ {pkg.advance?.toLocaleString()}</span>
                    </div>
                    <div className="text-gray-700 text-sm md:text-base">
                      <span className="font-instrumentsans font-medium">Price per person: </span>
                      <span className="font-instrumentsans text-[#1c5d5e] font-semibold">₹ {pkg.price?.toLocaleString()}</span>
                    </div>
                  </div>
                  <p className="font-instrumentsans text-xs md:text-sm text-[#1c5d5e] text-center">Pay only booking advance to confirm your spot</p>
                </div>

                <div className="prose max-w-none mb-6">
                  <p className="font-instrumentsans text-gray-800 leading-relaxed">{pkg.description}</p>
                </div>
                ...
                {pkg.itenary && (
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-4">
                      <span className="font-instrumentsans text-lg font-semibold">Your Journey</span>
                      ...
                      {pkg.itenary.split('\n').map((item: string, index: number) => {
                        ...
                        return (
                          <div key={index} className="flex items-center space-x-3 bg-[#1c5d5e]/5 rounded-lg p-0">
                            <div className="font-instrumentsans font-semibold semibold flex-shrink-0 w-12 h-14 bg-[#1c5d5e] rounded-md flex items-center justify-center text-white text-md ml-1">
                              {`D${index + 1}`}
                            </div>
                            <div className="font-instrumentsans flex-1 min-w-0 flex flex-col justify-center">
  ...
  <div className="p-4">
    <div className="flex flex-row gap-2 items-stretch">
      ...
      <Link
        to={`/package/${pkg.id}`}
        onClick={onClose}
        className="font-instrumentsans font-semibold flex-1 min-w-0 bg-white border border-[#1c5d5e] hover:bg-[#164445] hover:text-white text-gray-800 font-medium py-3 px-4 rounded-lg flex items-center justify-center text-[#1c5d5e] text-sm"
      >
      ...
      <button
        ...
        className={`font-instrumentsans font-semibold flex-1 min-w-0 py-3 px-4 rounded-lg transition-colors text-sm flex items-center justify-center ${
          !selectedDate ||
          Object.keys(pkg.start_date_2).every(
            (date) => new Date(date) < new Date(new Date().setHours(0, 0, 0, 0))
          )
    ...
    {selectedDate &&
      !Object.keys(pkg.start_date_2).every(
        ...
        <p className="font-instrumentsans text-sm text-[#1c5d5e] mt-2 text-center">
          Click 'Book Now' to reserve your spot through WhatsApp •{' '}
          <span className="font-instrumentsans font-bold">PAY ₹ {pkg.advance?.toLocaleString() || 1} NOW</span>
        </p>
      )}
  </div>
</div>
